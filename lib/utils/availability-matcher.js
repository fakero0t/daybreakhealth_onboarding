...
export function matchAvailability(preferences, availabilityData, userTimezone) {
  ...
  for (const avail of availabilities) {
    ...
    const availDay = availStart.getDay()

    // Filter by specific dates if provided
    if (!matchesSpecificDates(availStart, preferences.specificDates)) {
      continue
    }
    ...
    if (preferences.timeRanges && preferences.timeRanges.length > 0) {
      for (const timeRange of preferences.timeRanges) {
        ...
        const overlap = calculateTimeOverlap(userStart, userEnd, availStart, availEnd)
    ...
    // Calculate date constraint score
    const dateScore = calculateDateConstraintScore(availStart, preferences.dateConstraints)
  ...
  const topMatches = matchedSlots.slice(0, 5)
