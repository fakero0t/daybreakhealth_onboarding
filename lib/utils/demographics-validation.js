...
export function validateDemographics(data, isPartial = false) {
  ...
  // Email field (255 char limit)
  if (data.email !== undefined && data.email !== null) {
    const error = validateEmailField(data.email, 'email', 'Email');
 ...
 * @returns {object|null} Error object or null
 */
function validateEmailField(value, field, label) {
  if (typeof value !== 'string') {
    return {
      field,
      message: `${label} must be a string`
  ...
  if (value.length > 255) {
    return {
      field,
      message: `${label} must not exceed 255 characters (current: ${value.length})`
    };
  }
